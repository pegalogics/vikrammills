...
class iseedAll extends Command
{
    ...
    {
        $query =  \DB::select('SHOW TABLES WHERE Tables_in_' . env('DB_DATABASE', 'vikrammills') . ' ' . 'NOT LIKE "migrations"');
        ...
        $collection = new \Illuminate\Support\Collection($query);
        $tables = $collection->implode('Tables_in_' . env('DB_DATABASE', 'vikrammills'), ',');
        $this->info('Calling iseed for all tables except migrations ...');
